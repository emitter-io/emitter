...
{
    /// <summary>
    /// Represents the DISCONNECT message from client to broker.
    /// </summary>
    internal sealed class MqttDisconnectPacket : MqttPacket
    {
        #region Static Members

        private static readonly ConcurrentPool<MqttDisconnectPacket> Pool =
            new ConcurrentPool<MqttDisconnectPacket>("MQTT Disconnect Packets", (c) => new MqttDisconnectPacket());
        ...
        public override unsafe MqttStatus TryRead(BufferSegment buffer, int offset, int length)
        {
            ...
            if (this.ProtocolVersion == MqttProtocolVersion.V3_1_1
                && (*pBuffer & MSG_FLAG_BITS_MASK) != MQTT_MSG_DISCONNECT_FLAG_BITS)
