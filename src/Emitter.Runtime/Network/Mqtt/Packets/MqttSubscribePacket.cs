...
namespace Emitter.Network
{
    ...
    /// </summary>
    internal sealed class MqttSubscribePacket : MqttPacket
    {
        #region Static Members

        private static readonly ConcurrentPool<MqttSubscribePacket> Pool =
            new ConcurrentPool<MqttSubscribePacket>("MQTT Subscribe Packets", (c) => new MqttSubscribePacket());
        ...
        public override unsafe MqttStatus TryRead(BufferSegment buffer, int offset, int length)
        {
            ...
            if (this.ProtocolVersion == MqttProtocolVersion.V3_1_1
                && (*pBuffer & MSG_FLAG_BITS_MASK) != MQTT_MSG_SUBSCRIBE_FLAG_BITS)
            ...
            do
            {
                // Read the channel string
                var channel = ReadString(pBuffer, ref offset);

                // Read the topic string and push it to the channels list
                this.Channels.Add(channel);

                // We simply skip the QoS level here, as in Emitter we don't take this
                // part of MQTT protocol into account and have a QoS = 0 always.
                ++offset;
            } while (offset < length);
