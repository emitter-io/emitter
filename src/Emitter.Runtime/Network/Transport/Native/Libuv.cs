...
{
    internal class Libuv
    {
        public Libuv()
        {
            IsWindows = Platform.System == OS.Windows;
            ...
            _uv_tcp_keepalive = NativeMethods.uv_tcp_keepalive;
            ...
            [System.Runtime.CompilerServices.CallerMemberName] string memberName = "")
        {
            if (statusCode < 0)
            {
                ...
                error = new UvException(
                    "Error " + statusCode + " " + errorName + " " + errorDescription,
                    memberName
                    );
            }
            else
        ...
        protected delegate int uv_tcp_connect_func(UvConnectRequest req, UvTcpHandle handle, ref SockAddr addr, uv_connect_cb connect_cb);

        protected uv_tcp_connect_func _uv_tcp_connect;
        ...
        protected Func<UvTcpHandle, int, uint, int> _uv_tcp_keepalive;

        public void tcp_keepalive(UvTcpHandle handle, bool enable, uint delay)
        {
            handle.Validate();
            Check(_uv_tcp_keepalive(handle, enable ? 1 : 0, delay));
        }
        ...
        private static class NativeMethods
        {
            ...
            [DllImport("libuv", CallingConvention = CallingConvention.Cdecl)]
            public static extern int uv_tcp_keepalive(UvTcpHandle handle, int enable, uint delay);
