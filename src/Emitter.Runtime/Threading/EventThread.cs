...
{
    /// <summary>
    /// Represents a UVLib event thread.
    /// </summary>
    public sealed class EventThread
    {
        ...
        internal UvLoopHandle Loop { get { return _loop; } }
        ...
        public int WorkRunning
        {
            get { return this._workRunning.Count; }
        }

        public int WorkAdding
        {
            get { return this._workAdding.Count; }
        }
            ...
            if (_thread.IsAlive)
            {
                ...
                try
                {
                    ...
                    if (!_thread.Join(stepTimeout))
                    {
                        ...
                        if (!_thread.Join(stepTimeout))
                        {
                            //_log.LogError(0, null, "EmitterThread.Stop failed to terminate libuv thread.");
                        }
                    }
                }
        ...
        private void Post(Action<EventThread> callback)
        {
            Post(thread => callback((EventThread)thread), this);
        }
