...
const strInvalidID = "security: invalid ID"
...
func TestFromString(t *testing.T) {
	id, err := ParseID("9m4e2mr0ui3e8a215n4g")
...
func TestIDDriverScanError(t *testing.T) {
	id := ID{}
	err := id.Scan(0)
	assert.EqualError(t, err, "security: scanning unsupported type: int")
	err = id.Scan("0")
	assert.EqualError(t, err, strInvalidID)
}
...
func BenchmarkFromString(b *testing.B) {
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			_, _ = ParseID("9m4e2mr0ui3e8a215n4g")
		}
	})
}
